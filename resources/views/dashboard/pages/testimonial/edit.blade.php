...
<div class="flex-1 px-2 md:px-3 mt-14 md:mt-0 bg-indigo-50 min-h-screen pt-2">
    ...
    <div class="w-full flex md:w-2/3 m-auto">
        <a href="/dashboard/testimonial" class="text-sm md:text-base py-2 px-2 md:py-2 md:px-3 bg-green-500 text-white rounded-md shadow-sm mb-2 flex gap-1 justify-center items-center hover:bg-green-600">
    ...
    <form id="fileUploadForm" method="POST" action="/dashboard/testimonial/{{ $testimonial->slug }}" enctype="multipart/form-data" class="md:w-2/3 w-full m-auto">
        @method('put')
        @csrf
        {{-- nama testimonial --}}
        <label for="name" class="tracking-wide text-sm font-medium">Nama Testimonial</label>
        <input id="name" type="text" name="name" class="w-full py-1 px-2 md:py-2 md:px-4 border-2 rounded-sm mb-2" value="{{ old('name', $testimonial->name)}}" required>
        @error('name')
        ...
        <input id="slug" type="text" name="slug" readonly  class="w-full py-1 px-2 md:py-2 md:px-4 border-2 rounded-sm mb-2" value="{{ old('slug', $testimonial->slug) }}" required>
        @error('slug')
            <p class="text-sm text-red-600">{{ $message }}</p>
        @enderror

        <label for="body" class="tracking-wide text-sm font-medium">Nama Testimonial</label>
        <input id="body" type="text" name="body" class="w-full py-1 px-2 md:py-2 md:px-4 border-2 rounded-sm mb-2" value="{{ old('body', $testimonial->body)}}" required>
        @error('body')
            <p class="text-sm text-red-600">{{ $message }}</p>
        @enderror
