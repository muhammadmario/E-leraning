...
<div class="flex-1 px-2 md:px-3 mt-14 md:mt-0 bg-indigo-50 min-h-screen pt-2">
    ...
    <form id="fileUploadForm" method="POST" action="/dashboard/kelas/{{ $classroom->slug }}" enctype="multipart/form-data" class="md:w-2/3 w-full m-auto">
        @method('put')
        @csrf
        {{-- nama kelas --}}
        ...
        @if ($classroom->image)
            <img src="{{ asset('storage/'.$classroom->image) }}" class="h-48 aspect-[5/3] img-preview block mb-3">
        @else
