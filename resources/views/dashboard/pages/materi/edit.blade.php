...
<div class="flex-1 px-2 md:px-3 mt-14 md:mt-0 bg-indigo-50 min-h-screen pt-2">
    ...
    <form id="fileUploadForm" method="POST" action="/dashboard/materi/{{ $lesson->slug }}" enctype="multipart/form-data" class="md:w-2/3 w-full m-auto">
        @method('put')
        @csrf
        ...
        <input id="slug" type="text" name="slug" readonly  class="w-full py-1 px-2 md:py-2 md:px-4 border-2 rounded-sm mb-2" value="{{ old('slug', $lesson->slug) }}" required>
        @error('slug')
            <p class="text-sm text-red-600">{{ $message }}</p>
        @enderror
        ...
        <input id="user_id" type="hidden" name="user_id" class="w-full py-1 px-2 md:py-2 md:px-4 border-2 rounded-sm mb-2 hidden" value="{{ auth()->user()->id }}" required readonly>
        @error('user_id')
            <p class="text-sm text-red-600">{{ $message }}</p>
        @enderror
