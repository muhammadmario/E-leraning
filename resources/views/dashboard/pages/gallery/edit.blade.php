...
<div class="flex-1 px-2 md:px-3 mt-14 md:mt-0 bg-indigo-50 min-h-screen pt-2">
    ...
    <form id="fileUploadForm" method="POST" action="/dashboard/gallery/{{ $gallery->slug }}" enctype="multipart/form-data" class="md:w-2/3 w-full m-auto">
        ...
        <input id="name" type="text" name="name" class="w-full py-1 px-2 md:py-2 md:px-4 border-2 rounded-sm mb-2" value="{{ old('name', $gallery->name)}}" required>
        @error('name')
        ...
        @enderror
         ...
         <input id="body" type="text" name="body" class="w-full py-1 px-2 md:py-2 md:px-4 border-2 rounded-sm mb-2" value="{{ old('body', $gallery->body)}}" required>
         @error('body')
             <p class="text-sm text-red-600">{{ $message }}</p>
         @enderror
        ...
        <input id="slug" type="text" name="slug" readonly  class="w-full py-1 px-2 md:py-2 md:px-4 border-2 rounded-sm mb-2" value="{{ old('slug', $gallery->slug) }}" required>
        @error('slug')
            <p class="text-sm text-red-600">{{ $message }}</p>
        @enderror
        ...
        @if ($gallery->image)
            <img src="{{ asset('storage/'.$gallery->image) }}" class="h-15 aspect-[4/3] img-preview block mb-3">
        @else
