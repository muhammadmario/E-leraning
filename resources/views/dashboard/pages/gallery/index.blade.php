...
<div class="flex-1 px-2 md:px-3 mt-14  md:mt-0 bg-indigo-50 min-h-screen pt-2">
    ...
    <div class="w-full flex justify-end">
        <a href="/dashboard/gallery/create" class="py-2 px-3 bg-green-500 text-white rounded-md shadow-sm mb-2 flex gap-1 justify-center items-center hover:bg-green-600">
        ...
        <table class="table-auto w-full">
            ...
            <tbody> 
                ...
                <tr class="{{ $loop->index % 2 == 1 ? "bg-white" : "" }}">
                    <td class="p-3 ">
                        <img src="{{ asset('storage/'.$gallery->image) }}" alt="{{ $gallery->name }}" class="h-24 aspect-[4/3]">
                    </td>
                    <td class="p-3 text-sm text-gray-700">{{ $gallery->name }}</td>
                    <td class="p-3 text-sm text-gray-700">{{ Str::limit($gallery->body, 40, ) }}</td>
                    <td class="p-3 text-sm text-gray-700">
                        <div class="flex flex-row gap-2">
                            <a href="/dashboard/gallery/{{ $gallery->slug }}/edit">
                            ...
                            <form action="/dashboard/gallery/{{ $gallery->slug }}" method="POST">
                                @method('delete')
                                @csrf
